...
> **Version**: 0.1.0 | **Tests**: run `pytest --collect-only -q | tail -1` for the live collected count (hand-typed counts are not maintained here per hft-rules §11; was 628/43-files at the 2026-05-30 #PY-263 cycle, +38 tests / +2 files from the B1-B4 engine-realism `e08fb89` + deep-ITM demo `6295d82` commits) | **Last Updated**: 2026-07-07 (Phase-2 TRUTH doc pass: stamps reconciled to this single header stamp [the conflicting 2026-06-19 footer stamp removed]; hand-typed test counts → run-command pointers; the D1/F3 CROSS-REPO `backtest_deeplob.py`-default action item marked CLOSED [hft-ops removed the default 2026-05-31, commit `e89c2fd` — see State at HEAD + `VALIDATION_FINDINGS_2026_05_30.md` §9.3]; `demos/` added to the §2 tree. Prior stamp 2026-05-30 = the #PY-263 annualization-closure cycle — its full detail lives in the State-at-HEAD entry below; substantive content since runs through the 2026-07-05 FINDING-046/114 cost-honesty entry + the 2026-07-07 doc-alignment commits `0c96a3d`/`273cee4`/`d721139`)  
...
│
├── demos/               # Runnable research demos
│   └── deep_itm_monthly_direction_null_demo.py  # FIND-049 (2026-06-19): ~1-month intraday deep-ITM option = the direction null (companion test: tests/test_engine/test_deep_itm_monthly_demo.py)
│
